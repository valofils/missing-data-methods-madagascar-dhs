...
body <- vapply(body, function(line) {
  ...
  new <- vapply(toks, function(tok) {
    nums <- strsplit(gsub("[][]", "", tok), ",")[[1]]
    paste0("\\cite{", paste0("ref", nums, collapse = ","), "}")
  }, character(1))
...
bib_block <- c("\\begin{thebibliography}{99}",
               paste0("\\bibitem{ref", ref_num, "} ", esc(ref_txt)),
               "\\end{thebibliography}")
